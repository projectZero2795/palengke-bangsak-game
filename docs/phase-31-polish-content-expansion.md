...
Android, power-ups, tournaments, Photon transport, or anti-cheat behavior. Each
approved follow-up phase must still be implemented, tested, pushed, and
reviewed separately.
...
| 34 | Mobile, Android, and accessibility baseline | Responsive HUD/input pass, touch target and safe-area handling, reconnect/background behavior, reference-device performance budget, Android build pipeline, readable text/contrast, reduced-motion option, and non-audio cues. | WebGL desktop and the agreed Android reference device can complete a round with readable controls and no critical layout overlap. |
...
Phase numbers become implementation commitments only after this Phase 31 plan is
approved. The existing selectable-component registry issue is consumed by
Phase 37, and the existing random power-up issue is consumed by Phase 39.
...
- Phase 34: Android minimum version, reference phone/tablet, orientation, and
  target store/distribution path.
...
- Maps, art, skins, badges, events, sound, Android, anti-cheat, tournaments,
  power-ups, and component selection have an ordered implementation path.
