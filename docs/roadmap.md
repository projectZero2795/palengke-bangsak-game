...
| 29C | Urgent canonical cutover | Make `bangsak` canonical and redirect old `games` paths to it. | In progress; new URL must retain the full guest/auth/Photon checks and old URLs must preserve paths and queries when redirecting. |
